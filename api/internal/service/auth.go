...
func (s *authService) ExchangeCodeForToken(ctx context.Context, code, codeVerifier string) (dto.TokenDetails, error) {
	...
	existingUserId, err := s.userRepo.GetUserByOIDCSub(ctx, claims.Sub)
	...
	if existingUserId == uuid.Nil {
		userID, err = s.userRepo.InsertNewUser(ctx, claims.Sub, claims.Email)
		if err != nil {
			return dto.TokenDetails{}, errors.New("user insert failed: " + err.Error())
		}
		// create default org
